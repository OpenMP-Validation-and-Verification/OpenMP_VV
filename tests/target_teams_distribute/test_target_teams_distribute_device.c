...
int main() {
  ...
  if (!sum_errors){
      ...
      OMPVV_INFOMSG("Test failed with offloading %s", (isOffloading ? "enabled" : "disabled"));
  }
  for (int dev = 0; dev < num_devices; ++dev){
      if (!errors[dev] && num_teams[dev] == 1){
          OMPVV_INFOMSG("Test operated with one team. Parallelism of teams distribute can't be guarnunteed.");
      }
      else if (errors[dev]){
          OMPVV_INFOMSG("Test failed with device %d", dev);
      }
  }
