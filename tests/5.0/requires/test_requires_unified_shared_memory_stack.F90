!===--- test_requires_unified_shared_memory_stack.F90 --------------------------===//
!
...
PROGRAM test_requires_unified_shared_memory_stack
   USE iso_fortran_env
   USE ompvv_lib
   USE omp_lib
   implicit none
...
!$omp requires unified_shared_memory
  ...
  OMPVV_TEST_VERBOSE(unified_shared_memory_stack() .NE. 0)

  OMPVV_REPORT_AND_RETURN()

CONTAINS
  INTEGER FUNCTION unified_shared_memory_stack()
    INTEGER:: errors, i
    ...
    unified_shared_memory_stack = errors
  END FUNCTION unified_shared_memory_stack
END PROGRAM test_requires_unified_shared_memory_stack
