...
PROGRAM test_loop_nested_device
  USE iso_fortran_env
  USE ompvv_lib
  USE omp_lib
  implicit none
...
CONTAINS
  INTEGER FUNCTION test_loop_nested_teams()
    ...
    !$omp target teams num_teams(OMPVV_NUM_TEAMS_DEVICE) thread_limit(OMPVV_NUM_THREADS_HOST) map(tofrom: x, y, z, num_teams)
    !$omp loop
    DO i = 1, N
       DO j = 1, N
          x(j,i) = x(j,i) + y(i)*z(i)
...
END PROGRAM test_loop_nested_device
