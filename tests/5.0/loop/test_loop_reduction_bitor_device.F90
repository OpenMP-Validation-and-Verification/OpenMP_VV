!===--- test_loop_reduction_bitor_device.F90 -------------------------------------===//
!
...
PROGRAM test_loop_reduction_bitor_device
  USE iso_fortran_env
  USE ompvv_lib
  USE omp_lib
  implicit none
    ...
    DO WHILE ( ((.not. have_true) .or. (.not. have_false)) .and. (num_attempts .lt. THRESHOLD) ) 
       ...
       DO x = 1, N
          ...
          DO y = 0, 15
             ...
             IF (randomNumber .gt. true_margin) THEN
                a(x) = a(x) + ISHFT(1, y)
                have_true = .TRUE.
             ELSE
                have_false = .TRUE.
...
END PROGRAM test_loop_reduction_bitor_device
