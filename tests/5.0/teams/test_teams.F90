...
! requested number of teams with requested number of threads were created.
!
!//===----------------------------------------------------------------------===//
...
CONTAINS
  INTEGER FUNCTION teams()
    ...
    OMPVV_WARNING_IF(num_teams(0) .eq. 1, "Test operated with one team, num_teams
requested is inconsistent with this result")
 
    OMPVV_ERROR_IF(num_teams(0) .le. 1, "omp_get_num_teams() reported a value
less than one.")

    OMPVV_WARNING_IF(num_threads(0) .eq. 1, "Team 0 reported only 1 thread. This
