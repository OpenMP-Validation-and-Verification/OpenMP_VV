...
int test_target_device_clause() {
  ...
  for (int dev = 0; dev < num_dev; ++dev) {
#pragma omp target enter data map(to: array[0:N]) device(dev)
#pragma omp target map (alloc: array[0:N]) device(dev)
  ...
  // recover the array from each of the device and make sure the value is dev;
  for (int dev = 0; dev < num_dev; ++dev) {
#pragma omp target exit data map(from: array[0:N]) device(dev) 
    for (int i = 0; i < N; ++i) {
