...
int test_omp_set_num_teams()
{
  ...
  if (num_teams[0] == 1) {
    ...
    errors++;
  } else if (num_teams[0] > OMPVV_NUM_TEAMS_DEVICE) {
    OMPVV_ERROR("omp_get_num_teams() reported a value greater than set.");
    errors++;
  }
...
int main() {
  ...
  OMPVV_TEST_AND_SET_VERBOSE(errors, test_omp_set_num_teams());
