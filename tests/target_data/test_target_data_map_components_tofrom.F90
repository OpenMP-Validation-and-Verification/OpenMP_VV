!===--test_target_data_map_components_tofrom.F90 - derived data type map default-===!
! 
! OpenMP API Version 4.5 Nov 2015
!
! This test checks the mapping of derived data types (components) behavior.
! with the tofrom modifier. Data should be copied over the device and
...
#define N 100
      ...
      PROGRAM test_target_data_map_components_tofrom
        USE iso_fortran_env
        USE ompvv_lib
        USE omp_lib
        implicit none
        ...
        OMPVV_TEST_VERBOSE(test_map_derived_type_tofrom() .ne. 0)

        OMPVV_REPORT_AND_RETURN()

        CONTAINS 
          ! Default mapping
          INTEGER FUNCTION test_map_derived_type_tofrom()

            OMPVV_INFOMSG("Testing tofrom mapping")
            ...
            !$omp target data map(tofrom: myStruct, cpyStruct) &
            !$omp map(tofrom: myStructArr(:), cpyStructArr(:))
            ...
            test_map_derived_type_tofrom = err_af - err_bf

          END FUNCTION test_map_derived_type_tofrom
      END PROGRAM test_target_data_map_components_tofrom
